# Plot Monthly Avg. Total Snowfall at WA Cascade Passes (2005-2024) and
...
create_directories <- function() {
  ...
  return(list(data_dir = data_dir, figures_dir = figures_dir))
}
...
create_pass_dataframe <- function() {
  ...
  pass_year <- 2005:2024
  return(data.frame(pass = pass, pass_num = pass_num, pass_year = pass_year))
}
...
get_snowfall <- function(df, data_dir) {
  ...
  if (!file.exists(snowfall_file)) {
    ...
    burl <- 
      "https://wsdot.com/Travel/Real-time/Service/api/MountainPass/SnowFallData?"
    
    # Construct URLs for each pass/year
    df_snowfall <- df %>%
      group_by(pass_num, pass_year) %>%
      mutate(
        url = map2_chr(.x = pass_num,
                       .y = pass_year,
                       ~ paste0(burl,
                                "MountainPassId=", .x,
                                "&",
...
ggsave(plot1,
       filename = here(dirs$figures_dir,
                       "monthly_avg_tot_snowfall_wa_cascade_passes.png"))

# Prepare data for bar plot
summarized_snow_data <- snow_data %>%
  group_by(pass, ENSO) %>%
  summarise(avg_tot_snowfall_in = mean(avg_tot_snowfall_in), .groups = "drop")
...
plot2 <- ggplot(summarized_snow_data,
  ...
  theme_minimal() +
  labs(title =
         paste0("Monthly Avg. Total Snowfall at WA Cascade Passes (",
                min(df_pass$pass_year), "-", max(df_pass$pass_year), ")"),
       caption = "Data Sources: WSDoT Snowfall report and GGWeather ENSO ONI",
       y = "Pass", x = "Monthly Avg. Total Snowfall (inches)")

ggsave(plot2,
       filename = here(dirs$figures_dir,
                       "monthly_avg_tot_snowfall_wa_cascade_passes_bar.png"))
